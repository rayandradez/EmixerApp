...
public class MessageService extends Service {
    ...
    /**
     * Aplica as configurações de volume e pan (balanço) ao MediaPlayer.
     * Esta é a versão corrigida que lida com o pan corretamente.
     */
    private void applyVolumeAndPan() {
        if (mediaPlayer == null) return;

        // Converte os valores de 0-100 para uma escala de 0.0-1.0
        float volume = mainVolume / 100.0f; // Volume geral (ex: 0.5 para 50%)
        // Converte o pan de -100 a 100 para uma escala de -1.0 a 1.0
        float pan = panValue / 100.0f;

        float leftVolume;
        float rightVolume;

        // Calcula o volume de cada canal com base no pan
        ...
        Log.d(TAG, "Aplicando Volume/Pan: Base=" + volume + ", Pan=" + pan + ", Final L=" + leftVolume + ", Final R=" + rightVolume);
